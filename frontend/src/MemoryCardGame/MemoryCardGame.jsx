...
const saveGameData = async (gameData) => {
  try {
    ...
    console.log("Game data saved successfully YES!!!", response.data);
  } catch (error) {
    console.error("Error saving game data:", error.response ? error.response.data : error.message);
  }
};

const listGameData = async (userData) => {
  try {
    const response = await axios.post("http://localhost:5000/api/memory/list", userData, {
      headers: { "Content-Type": "application/json" },
    });

    console.log("Game data listed successfully", response.data);
  } catch (error) {
    console.error("Error listing game data:", error.response ? error.response.data : error.message);
  }
};
